...
firmware, sitting here as a subdirectory rather than a page. Building it needs
an ESP-IDF toolchain, and its `firmware/` build tree is gitignored, so expect
this one directory to behave like code even though it lives among the docs. It
is worth reading even if you never run it, because it is the worked example of
...
export HW_HOST=              # SSH host or address for the device
